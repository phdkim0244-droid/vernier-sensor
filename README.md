...
- 그래프: 막대/꺾은선, 세로축 여러 값, **값 표시(자동·모두·숨김)**, **크기(보통·크게·전체 화면)**, **세로축 0부터/자동 범위**, **이미지(PNG) 저장**.
- 연결 개수 제한 없음. 여러 개는 USB 권장(전원 공급형 USB 허브로 8개 안팎), 블루투스는 4개 이내 권장.
- 📡 채널 선택: 조도·색도 센서(GDX-LC)처럼 채널이 여러 개인 기기는 조도·빨강·초록·파랑·UVA·UVB 등을 켜고 끌 수 있음(기기 종류별로 브라우저에 기억). 채널을 바꾸면 측정을 멈췄다 다시 시작해 채널 마스크를 갱신함. LED 제어는 내장 라이브러리에 명령이 없어 미지원.
- 🎨 색 미리보기: 615/525/465 nm(빨강·초록·파랑) 세 값의 비율로 색을 보여 줌. 센서의 파장별 감도 차이로 파랑이 늘 세게 나오므로 **⚪ 흰색 기준 맞추기**(흰 종이에 대고 누름, 기기 종류별 저장)로 보정 가능.

## 체험 모드
센서가 없을 때 주소 뒤에 `?demo`를 붙이면(예: `index.html?demo`) 가짜 조도·색도(6채널) 센서와 온도 센서로 채널 선택까지 시험해 볼 수 있습니다.
